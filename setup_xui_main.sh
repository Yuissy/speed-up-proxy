...
# === 2. ПРИВЯЗЫВАЕМ К LOCALHOST И ЗАДАЁМ ПУТЬ ===
...
# === 3. СОЗДАЁМ INBOUND ЧЕРЕЗ ВНЕШНИЙ СКРИПТ ===
...
bash <(curl -Ls https://raw.githubusercontent.com/Yuissy/speed-up-proxy/main/create_inbound.sh) \
    "$PANEL_PORT" "$WEB_BASE" "$DOMAIN" "$SECRET_PATH" "$CLIENT_UUID" || error "Не удалось создать inbound"
