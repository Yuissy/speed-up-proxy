...
# Fixed by Gemini: repaired geo-dependencies, repository logic & 9-byte bugs.
set -euo pipefail
...
run_server2() {
    ...
    # Принудительно создаем папку для скриптов перед вызовом
    mkdir -p "$SCRIPT_DIR"
    # Накатываем базы
    install_geo_files
...
run_server1() {
    ...
    echo
    read -rp "$(question 'Домен (A-запись → этот server): ')" domain
